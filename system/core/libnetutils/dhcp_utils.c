...
/* Value of prefix in static_do_request function when
 * static Network prefixLength is null.
 */
#define DEFAULT_STATIC_PREFIX_LENGTH 24
                    ...
                    const int prefixLength)
{
    ...
    if (prefixLength == 0) prefix = DEFAULT_STATIC_PREFIX_LENGTH;
    ...
    snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-S ip_address=%s/%d -S routers=%s %s", DAEMON_NAME,
        interface, ipaddr, prefix, gateway, interface);
